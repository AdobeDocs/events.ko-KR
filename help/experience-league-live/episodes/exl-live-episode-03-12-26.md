---
title: Adobe Journey Optimizer에서 Campaign Orchestration 알아보기
description: Journey Optimizer의 campaign orchestration이 대상 생성부터 시작까지 일대다 다중 채널 마케팅 프로그램을 관리하는 방법을 알아봅니다.
kt: KT-19994
solution: Journey Optimizer
feature: Audiences
topic: Security
event-cta-url-live: https://www.youtube.com/live/mPVvZIbkJq8
event-start-time: 2026-03-03 08:30-8
event-guests: Sandra Hausmann, Bridgette Darling, Chakravarthy Kalva, Lee Henderson
exl-id: 5ad4e649-d00a-4934-bfee-13d358fdad11
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '145'
ht-degree: 0%

---

# Adobe Journey Optimizer에서 Campaign Orchestration 알아보기

Journey Optimizer의 campaign orchestration이 대상 생성부터 시작까지 일대다 다중 채널 마케팅 프로그램을 관리하는 방법을 알아봅니다.

Campaign 오케스트레이션 캔버스와 관계형 스키마를 통해 복잡한 캠페인과 고객 관계를 관리하는 기반, 복잡한 데이터에서 적합한 대상자를 타깃팅하는 다중 엔티티 세그멘테이션, 온디맨드 대상자 세분화 및 정확성을 위한 사전 전송 횟수, 그리고 현재 Journey Optimizer 내에서 이 기능이 원활하게 작동하는 방식에 대해 살펴보았습니다.

>[!VIDEO](https://video.tv.adobe.com/v/3481409/?quality=12&learn=on)

이 주제에 대한 대화를 계속하려면 [Experience League 커뮤니티 토론](https://experienceleaguecommunities.adobe.com/adobe-journey-optimizer-15/post-session-discussion-experience-league-live-get-to-know-campaign-orchestration-in-adobe-journey-optimizer-249161?profile.language=ko){target="_blank"}을 참조하세요.
