...
Bryan, Eric 및 Doug와 함께 Customer Journey Analytics(CJA)를 사용하여 빠르게 전환하는 방법에 대해 논의합니다. 자동화된 프로세스를 사용하여 Adobe Analytics에서 CJA으로 데이터 및 분석을 이동하는 방법과 프로세스 중에 고려해야 할 사항에 대해 알아봅니다. 그리고 물론, 그들은 그 과정에서 건강한 양의 재미있는 팁과 요령을 가질 것입니다.
...
* Adobe Analytics에서 Customer Journey Analytics으로 데이터를 가져오는 방법에는 Analytics 데이터 커넥터(ADC)와 웹 SDK의 두 가지가 있습니다.
* ADC를 사용하면 보고서 세트의 데이터를 분석을 위해 Adobe Experience Platform에 복사할 수 있지만, 웹 SDK은 데이터를 Adobe Experience Platform으로 직접 전송합니다.
...
* Adobe Analytics 관리자가 프로젝트를 CJA으로 마이그레이션할 수 있는 &quot;구성 요소 마이그레이션&quot;이라는 새로운 도구가 있습니다.
* 프로젝트를 마이그레이션할 때 적용된 세그먼트 또는 계산된 지표는 물론 테이블의 모든 구성 요소가 CJA으로 이동됩니다.
* CJA에 존재하지 않는 구성 요소를 다목적 캐치 또는 파생 필드를 사용하여 매핑할 수 있는 매핑 프로세스가 있습니다.
* CJA에 존재하지 않는 요소에 대해 다목적 캐치(catch-all)를 만든 다음 대상 프로젝트에서 편집하는 것이 좋습니다.
* 이전에는 CJA으로 마이그레이션할 때 계산된 지표 및 세그먼트를 다시 생성해야 한다고 생각되었지만, 이제 마이그레이션할 수 있는 옵션이 있습니다.
