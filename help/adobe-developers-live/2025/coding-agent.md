---
title: AI 코딩 쇼다운 - 좋은, 나쁜, 추한
description: Adobe Experience Manager과 함께 작동하는 AI 코딩 에이전트를 탐색하여 실제 시나리오에서 강점과 과제를 확인하십시오.
role: Developer
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 1603
last-substantial-update: 2025-12-10T00:00:00Z
jira: KT-19973
source-git-commit: 76b4e64093ebad269534b65c41c4bda1a6bb468f
workflow-type: tm+mt
source-wordcount: '183'
ht-degree: 1%

---


# AI 코딩 대결: 좋은 것, 나쁜 것, 못생긴 것

AI 코딩 에이전트가 실제 Edge Delivery Services 프로젝트에서 어떻게 작동하는지 알아봅니다. 솔루션의 강점, 과제 및 개발자 경험에 미치는 영향에 대해 알아봅니다. Adobe Developers Live 2025의 이번 세션에서는 실용적인 자동화와 &#39;바이브 코딩&#39;을 선보이며 AI가 우수한 분야와 고군분투하는 분야를 조명한다. Lars Trieloff와 함께 Agentic Engineering을 테스트하고 선도적인 AI 코딩 에이전트를 비교하여 디지털 경험의 미래에 대한 통찰력을 제시합니다.

>[!VIDEO](https://video.tv.adobe.com/v/3477655/?captions=kor&learn=on&enablevpops)

이 세션 - 코딩 에이전트 쇼다운: 좋은 것, 나쁜 것, 못생긴 것 - 은 에이전트 엔지니어링을 테스트합니다. 라스 트리엘로프는 실제 Edge Delivery Services 프로젝트에서 선두적인 AI 코딩 에이전트를 비교하여 개발자 경험이 에이전트 경험을 충족하는 위치를 보여줍니다. AI가 빛을 발하는 곳, 고군분투하는 곳, 실용적인 자동화와 &#39;바이브 코딩&#39;을 구분하는 것을 시청하십시오. San Jose에서 생방송으로 녹화됨.

Adobe Developers Live 2025를 지원해준 우리의 후원자인 Algolia와 Ensemble에게 특별한 감사를 드립니다.

## 다음 단계

* [Experience League](https://adobe.ly/47ss08A)에서 대화 계속
* [예정된 이벤트](https://developerevents.adobe.com) 검색
