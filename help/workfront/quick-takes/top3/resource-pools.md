---
title: 리소스 풀의 주요 이유
description: 효율적인 프로젝트 관리 및 보고를 위해 Adobe Workfront에서 리소스 풀을 사용하는 창의적인 방법을 살펴보십시오.
feature: Reports and Dashboards, Strategic Planning
topic: Administration, Integrations
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 271
last-substantial-update: 2025-12-17T00:00:00Z
jira: KT-18648
exl-id: f0f8a3ea-d7db-47b0-ba63-b25cbfb7ca15
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '210'
ht-degree: 10%

---

# 리소스 풀의 상위 3가지 이유

&quot;온디맨드 Workfront CSM&quot;이 새로운 Top 3 비디오로 돌아왔습니다! 이번에는 리소스 풀과 이를 활용하는 창의적인 방법에 대해 살펴보겠습니다. 이 항목은 항상 토론을 고무하므로 비디오를 확인하십시오.

>[!VIDEO](https://video.tv.adobe.com/v/3470071/?learn=on&enablevpops)

리소스 풀을 시작하시겠습니까? 먼저, 그것들은 무엇입니까?

* 리소스 풀은 프로젝트를 완료하는 데 동시에 필요한 사용자들의 집합입니다.
* 리소스 풀은 프로그램 및 포트폴리오의 모든 프로젝트와 연결될 때 여러 프로젝트, 프로그램 또는 포트폴리오에서 공유할 수 있습니다.
* 사용자로 리소스 풀을 채운 후 프로젝트 및 템플릿에 연결할 수 있으며 프로젝트, 템플릿 또는 사용자에 필요한 수만큼 리소스 풀을 연결할 수 있습니다.

## 추가 리소스

* 자세한 내용은 [리소스 풀 개요 문서](https://experienceleague.adobe.com/ko/docs/workfront/using/manage-resources/resource-planning-in-adobe-workfront/resource-pools/work-with-resource-pools)를 참조하세요.
* 더 많은 정보를 찾고 계십니까? [Experience League 커뮤니티 게시물](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/video-top-3-reasons-for-resource-pools/td-p/663866?profile.language=ko)을 확인해 보세요!
