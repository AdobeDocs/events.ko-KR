---
title: 사용자 지정 Forms에서 이미지를 사용해야 하는 주요 이유
description: 이미지, PDF 또는 비디오를 사용자 정의 양식에 추가하는 것이 사용자 참여도와 커뮤니케이션을 향상시키는 이유를 알아보십시오.
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 171
last-substantial-update: 2025-12-10T00:00:00Z
jira: KT-19741
source-git-commit: df552b4f2b2fa1d8e4e8941aa9d58acecc60dd2b
workflow-type: tm+mt
source-wordcount: '115'
ht-degree: 0%

---


# 사용자 지정 Forms에 이미지를 추가해야 하는 상위 3가지 이유

요청 대기열에 대한 사용자 정의 양식에 이미지, 비디오 또는 PDF를 포함해야 하는 이유를 강조 표시하는 이 상위 3개 비디오 에디션을 확인하십시오.

여기에는 커뮤니케이션을 개선하고 최종 사용자에게 보다 흥미로운 요청을 작성할 수 있는 재미있고 기능적인 팁이 모두 포함되어 있습니다.

>[!VIDEO](https://video.tv.adobe.com/v/3477059/?captions=kor&learn=on&enablevpops)

추가 리소스는 [Experience League 커뮤니티 게시물](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/video-top-3-reasons-to-add-an-image-to-custom-forms/m-p/576903?profile.language=ko#M374)을 확인하세요!

새로운 아이디어와 창의적인 접근 방식에 대해 배우는 것이 좋다면 [Experience League 이벤트 페이지](https://experienceleague.adobe.com/ko/events)에서 예정된 워크숍에 등록하는 것을 잊지 마세요.
