...
사용자에게 보고 ProofHQ에 대한 액세스 권한을 요청하면 왼쪽에 있는 섹션(예: 워크플로우, 연락처, 계정 설정 등)에 대한 액세스 권한도 제공됩니까?
...
다른 사용자가 변경 사항을 거부하거나 승인할 경우 새 결정을 트리거할 수 있는 방법이 있습니까? 즉, 증명 부서에서 보게 되면 프로젝트 관리자는 새로운 결정을 내려야 합니다. 이미 살펴보고 승인했더라도 말입니다. (프로젝트 관리자가 검토를 위해 교정을 기다리는 동안 작업을 수행하지 않도록 하려고 합니까?)
