---
title: Workfront 보고서로 봄 정리
description: Workfront Reports를 통해 효율적인 데이터 관리를 통해 봄 정리를 간소화하고 생산성을 향상시키는 방법을 살펴볼 수 있습니다.
feature: Reports and Dashboards, System Setup and Administration
topic: Administration
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3594
last-substantial-update: 2025-12-17T00:00:00Z
jira: KT-19772
exl-id: 93a99d21-f0db-45ae-ac37-6e996e0a8649
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '81'
ht-degree: 0%

---

# Workfront 보고서로 봄 정리

효과적인 봄 정리를 위해 Workfront 보고서를 사용하는 방법에 대해 알아봅니다. 데이터 감사 및 간소화, 검색 기능 개선 및 규정 준수를 통해 Workfront 인스턴스를 간소화할 수 있습니다. 생산성 및 거버넌스를 향상시키기 위해 프로젝트, 작업, 문제 및 사용자 프로필을 관리하는 전략을 살펴봅니다. 이 비디오는 보고서를 활용하여 깨끗하고 효율적인 Workfront 환경을 유지 관리하는 방법에 대한 통찰력을 제공합니다.

>[!VIDEO](https://video.tv.adobe.com/v/3477111/?learn=on&enablevpops)
